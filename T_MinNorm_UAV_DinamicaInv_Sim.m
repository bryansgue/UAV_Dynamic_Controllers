...
vref(:,k) = dynamic_compensation_UAV(vcp(:,k), vc(:,k), u_real(:,k), chi_real, K, G, L, ts);

%% 2) DINAMICA DEL UAV (VELOCIDAD Y POSICION)
u_real(:, k+1) = system_dynamic(chi_real, u_real(:,k), vref(:,k), psi(k), L,ts);

% Integracion numerica metodo Runge-Kutta 
h(:,k+1) = RK4(h(:,k),u_real(:,k+1),ts);
xu(k+1) = h(1,k+1);
yu(k+1) = h(2,k+1);
zu(k+1) = h(3,k+1);      
